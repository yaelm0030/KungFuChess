...
TEST_CASE("tick moves the clock forward by the given number of milliseconds") {
    GameEngine engine(Parser::parse_board({ "wK" }));
    engine.tick(1000);
    CHECK(engine.clock_ms() == 1000);
}

TEST_CASE("repeated ticks accumulate") {
    GameEngine engine(Parser::parse_board({ "wK" }));
    engine.tick(1000);
    engine.tick(250);
    engine.tick(1);
...
TEST_CASE("ticking zero milliseconds does not change the clock") {
    GameEngine engine(Parser::parse_board({ "wK" }));
    engine.tick(1000);
    engine.tick(0);
    CHECK(engine.clock_ms() == 1000);
...
TEST_CASE("a negative tick is ignored rather than rewinding the clock") {
    GameEngine engine(Parser::parse_board({ "wK" }));
    engine.tick(1000);
    engine.tick(-500);
    CHECK(engine.clock_ms() == 1000);
...
TEST_CASE("a move does not settle on the board until the clock reaches its arrival time") {
    GameEngine engine(Parser::parse_board({ "wK ." }));
    CHECK(engine.request_move(Position{ 0, 0 }, Position{ 1, 0 })); // one cell of travel time is needed

    engine.tick(GameEngine::kDefaultMoveMsPerCell - 1);
