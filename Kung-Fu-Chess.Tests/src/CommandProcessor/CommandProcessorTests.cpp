...
TEST_CASE("a click line dispatches to Controller::click and selects the clicked piece") {
    ...
    CommandProcessor processor(controller, out);
...
TEST_CASE("a print line dispatches to Controller::print and writes the board to the injected stream") {
    Controller controller(make_board());
    std::ostringstream out;
    CommandProcessor processor(controller, out);

    processor.run_line("print board");
